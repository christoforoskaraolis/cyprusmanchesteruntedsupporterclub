...
function buildText(matchKey: string): string {
...
Το αίτημά σας έχει καταχωρηθεί με επιτυχία και η προκαταβολή σας έχει ληφθεί.
...
function buildHtml(matchKey: string): string {
  const { matchName, matchDate } = formatFixtureMatchKeyForEmail(matchKey)
  ...
  <p>Το αίτημά σας έχει καταχωρηθεί με επιτυχία και η προκαταβολή σας έχει ληφθεί.</p>
...
export async function sendTicketDepositConfirmedEmail(options: {
  ...
  const text = buildText(options.matchKey)
  const html = buildHtml(options.matchKey)
